...
model, vis_processors, txt_processors = load_model_and_preprocess(
    name="img2prompt_vqa", model_type="base", is_eval=True, device=device
)
...
print(
    "Sample Question: {} \nSample Answer: {}".format(
        samples["questions"][:5], samples["answers"][:5]
    )
)
...
Img2Prompt_input = tokenizer(
    Img2Prompt, padding="longest", truncation=True, return_tensors="pt"
).to(device)
...
outputs = model.generate(
    input_ids=Img2Prompt_input.input_ids,
...
pred_answer = tokenizer.batch_decode(
    outputs.sequences[:, len(Img2Prompt_input.input_ids[0]) :]
)
